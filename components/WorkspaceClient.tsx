...
const parseMessages = (raw: unknown): Message[] => {
    ...
    return raw.filter((m): m is Message => 
        typeof m === "object" && m !== null && "role" in m && "content" in m,
    );
}

const parseFileData = (raw: unknown): FileData | null => {
...
const WorkspaceClient = ({initialPrompt, userCredits, userId, userPlan, workspace}: WorkspaceClientProps) => {
    ...
    //Abort controll ref which will stop the generation in between the chat
    const generatAbortRef = useRef<AbortController | null>(null);
    ...
    const handleGenerate = useCallback(
        ...
        //Create a fresh AbortController for this request
        const abortController = new AbortController();
        generatAbortRef.current = abortController;
